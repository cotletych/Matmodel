...
subtitle: "Задача о погоне - вариант 24"
...
Через определенный промежуток времени туман рассеивается, и лодка обнаруживается на расстоянии 11,4 км от катера. 
...
n=4.1 #разница в скорости
s=11.4#расстояние обнаружения
fi=pi*3/4 #угол движения

def f(tetha, r): #уравнение катера
    dr=r/sqrt(n**2 - 1)
    return dr
def f2(t): #уравнение лодки
    xt = tan(fi+pi)*t
    return xt
...
![траектории для первого случая](1.png){ #fig:001 width=70% height=70% }

Точка пересечения графиков - точка пересечения катера и лодки, имеет координаты
$$
 \begin{cases}
   \theta=315
   \\
...
![траектории для второго случая](2.png){ #fig:002 width=70% height=70% }
...
В первом случае нужно будет пройти гораздо меньшее расстояние, чтобы догнать браконьеров.
